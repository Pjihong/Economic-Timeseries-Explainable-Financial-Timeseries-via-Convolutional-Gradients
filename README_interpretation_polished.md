...
- Event Center of Mass(abs) 평균: **10.09**
- Control Center of Mass(abs) 평균: **11.56**
...
### 6-1. Center of Mass(abs) distribution

![COM distribution](outputs/posthoc/com_distribution.png)

- Event 평균 Center of Mass(abs): **10.09**
- Control 평균 Center of Mass(abs): **11.56**
- 검정: `p_raw = 0.110`, `p_fdr = 1.000`, `n_pairs = 4`

### 해석

Event의 Center of Mass(abs)가 더 작다는 것은, 중요도가 시간창의 **앞부분에 더 실린다**는 뜻이다.  
...
### 6-2. Focus(abs) distribution

![FOC distribution](outputs/posthoc/foc_distribution.png)

- Event Focus(abs) 평균: **2.91**
- Control Focus(abs) 평균: **2.63**
...
- Mean `|Standardized Mean Difference|` before: **0.51**
- Mean `|Standardized Mean Difference|` after: **0.75**
- Max `|Standardized Mean Difference|` before: **2.07**
- Max `|Standardized Mean Difference|` after: **2.85**

### 해석

이 그림은 post-hoc CAM 비교를 해석할 때 가장 중요하게 봐야 할 제한점을 보여준다.

- 많은 feature에서 `|Standardized Mean Difference| > 0.1` 을 크게 넘는다.
- 일부 feature는 `|Standardized Mean Difference| > 1` 수준으로 매우 크다.
