"""
event_warping.py — DTW, Distributional DTW, Embedding DTW, Event-weighted DTW.
"""
...
def _local_slice(T: int, t: int, k: int) -> slice:
    return slice(max(0, t - k), min(T, t + k + 1))
...
def wasserstein_1d(a: np.ndarray, b: np.ndarray) -> float:
    """SciPy-free Wasserstein-1 approximation via quantile matching."""
...
def quantile_l2_1d(a: np.ndarray, b: np.ndarray, n_q: int = 9) -> float:
    ...
    return float(np.sqrt(np.mean((np.quantile(a, qs) - np.quantile(b, qs)) ** 2)))
...
def energy_distance_1d(a: np.ndarray, b: np.ndarray) -> float:
    ...
    return float(max(2.0 * exy - exx - eyy, 0.0))
...
def _normalize_importance(
    g: Optional[np.ndarray], top_p: Optional[float] = None,
) -> Optional[np.ndarray]:
    if g is None:
        return None
...
def compute_cost_matrix_dtdw_1d(
    ...
    """Distributional DTW cost matrix for 1D sequences."""
    a, b = _as_1d(a), _as_1d(b)
    T = a.size
    if b.size != T:
        raise ValueError(f"DTDW expects equal length. got {T} vs {b.size}")
    C = np.full((T, T), np.inf, dtype=np.float64)
    _dist = {
        "wasserstein": wasserstein_1d,
        "energy": energy_distance_1d,
        "mmd": lambda ai, bj: mmd_rbf(ai.reshape(-1, 1), bj.reshape(-1, 1)),
        "quantile": lambda ai, bj: quantile_l2_1d(ai, bj, n_q=9),
    }
    dist_fn = _dist.get(method)
    if dist_fn is None:
    ...
    for i in range(T):
        ...
        for j in js:
            C[i, j] = dist_fn(ai, b[_local_slice(T, j, k)])
    return C
...
def compute_cost_matrix_embedding(
    ...
    """Embedding-space cost matrix."""
    A, B = _as_2d(Ea), _as_2d(Eb)
    if A.shape != B.shape:
    ...
    if k == 0 and method in ("l2", "cosine"):
        if method == "l2":
            ...
            D = 1.0 - np.clip(An @ Bn.T, -1.0, 1.0)
        if band is None:
            return D
        for i in range(T):
            j0, j1 = max(0, i - band), min(T, i + band + 1)
            C[i, j0:j1] = D[i, j0:j1]
        return C
...
def dtw_from_cost_matrix(
    ...
    """Standard DTW on a given cost matrix (T, T)."""
    ...
    for i in range(T):
        j_range = (
            range(T)
            if band is None
            else range(max(0, i - band), min(T, i + band + 1))
        )
        for j in j_range:
            if i == 0 and j == 0:
                continue
    ...
    while i > 0 or j > 0:
        ...
        _, ni, nj = min(cands, key=lambda x: x[0])
        i, j = ni, nj
        path.append((i, j))
    path.reverse()
...
def apply_event_weighting(
    ...
    if mode == "penalty":
        ...
        for i in range(T):
            for j in range(T):
                P[i, j] = float(penalty_fn(i, j, T)) if np.isfinite(C[i, j]) else np.inf
...
def dtdw_1d(a, b, k=3, method="wasserstein", band=5, normalize=True) -> DTWResult:
    C = compute_cost_matrix_dtdw_1d(a, b, k=k, method=method, band=band)
    return dtw_from_cost_matrix(C, band=band, normalize=normalize)


def wdtdw_1d(a, b, g_a, g_b, k=3, method="wasserstein", band=5,
             normalize=True, alpha=1.0, top_p=None, weight_mode="local",
             gamma=1.0) -> DTWResult:
    C0 = compute_cost_matrix_dtdw_1d(a, b, k=k, method=method, band=band)
    Cw = apply_event_weighting(C0, g_a=g_a, g_b=g_b, alpha=alpha,
                               top_p=top_p, mode=weight_mode, gamma=gamma)
    return dtw_from_cost_matrix(Cw, band=band, normalize=normalize)


def dtdw_embedding(Ea, Eb, method="l2", k=0, band=5, normalize=True) -> DTWResult:
    C = compute_cost_matrix_embedding(Ea, Eb, method=method, k=k, band=band)
    return dtw_from_cost_matrix(C, band=band, normalize=normalize)


def wdtdw_embedding(Ea, Eb, g_a, g_b, emb_method="l2", emb_k=0, band=5,
                    normalize=True, alpha=1.0, top_p=None, weight_mode="local",
                    gamma=1.0) -> DTWResult:
